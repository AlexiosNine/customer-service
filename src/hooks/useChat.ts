...
export function useChat(options: UseChatOptions) {
  ...
  // 发送满意度评分
  const submitSatisfaction = useCallback(async (sessionId: string, score: number, comment?: string) => {
    try {
      await fetch(`/api/sessions/${sessionId}/satisfaction`, {
        method: 'POST',
      ...
      setSessions(prev => prev.map(s =>
        s.id === sessionId
          ? { ...s, satisfaction: score, satisfactionComment: comment, status: 'resolved' }
          : s
      ));
    } catch (err) {
  ...
  // 手动转人工
  const triggerTransfer = useCallback(async (sessionId: string) => {
    try {
      ...
      setSessions(prev => prev.map(s =>
        s.id === sessionId ? { ...s, status: 'transferred' } : s
      ));
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                if (data.type === 'init') {
                  ...
                  // 重建 contentBlocks，用清理后的文本
                  const cleanBlocks: ContentBlock[] = [];
                  for (const block of contentBlocks) {
                    if (block.type === 'text') {
                      cleanBlocks.push({ type: 'text', text: cleanContent });
                      break;
                    } else {
                      cleanBlocks.push(block);
                    }
                  }
                  if (cleanBlocks.length === 0 && cleanContent) {
                    cleanBlocks.push({ type: 'text', text: cleanContent });
                  }
                  ...
                  setSessions(prev => prev.map(s => {
                    if (s.id === realSessionId) {
                      return {
                        ...
                        messages: s.messages.map(m => 
                          m.id === realAssistantMessageId 
                            ? { ...m, content: cleanContent, intent: intentType, contentBlocks: cleanBlocks }
                            : m
                        )
                      };
                    }
                ...
                } else if (data.type === 'transfer_triggered') {
                  // 自动转人工触发
                  setTransferTriggered(true);
                  setSessions(prev => prev.map(s => 
                    s.id === realSessionId ? { ...s, status: 'transferred' } : s
                  ));
